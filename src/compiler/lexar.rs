        ...
        for i in string.lines() {
            ...
            if tag.contains("\"") {
                ...
                if !i.split("\"").nth(0).unwrap().is_empty() {
                    tokens.push(Token {
                        keyword: String::from("TAG"),
                        value: i.split("\"").nth(0).unwrap().trim().to_string(),
                    });
                    tokens.push(Token {
                        keyword: String::from("MSG"),
                        value: i.split("\"").nth(1).unwrap().trim().to_string(),
                    });
                    //"[This is mypage]"
                }
